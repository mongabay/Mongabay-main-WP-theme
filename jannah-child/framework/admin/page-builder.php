...
function tie_get_builder_blocks( $block_id = false, $section_id = false , $block = array() ){
	...
	<li id="listItem_<?php echo esc_attr( $section_id .'-'. $block_id ) ?>" class="block-item parent-item <?php echo esc_attr( $block_class_name ) ?>">
		...
		<div class="tie-builder-content-area tie-popup-block tie-popup-window">
			...
			<div class="tie-section-title tie-section-tabs blocks-settings-tabs">
				<a href="#" data-target="basic-block-settings" class="active"><?php esc_html_e( 'Geeeneral', TIELABS_TEXTDOMAIN ) ?></a>
            ...
            // Obtenemos todos los post_types disponibles
            $post_types = get_post_types( array( 'public' => true ), 'objects' );

            // Preparaamos opciones para el campo select
            $post_types_options = array();
            foreach( $post_types as $post_type ) {
                $post_types_options[ $post_type->name ] = $post_type->label;
            }

            // Agregamos el campo de selección de post_type
            tie_page_builder_option(
                $block_id = $block_id,
                $section  = $section_id,
                $value    = $block['post_type'],
                array(
                    'name'    => esc_html__( 'Select Post Type', TIELABS_TEXTDOMAIN ),
                    'id'      => 'post_type',
                    'hint'   => esc_html__( 'Testing post_types selector.', TIELABS_TEXTDOMAIN ),
                    'type'    => 'select',
...
function tie_add_page_builder(){
	...
	<a class="tie-primary-button button button-hero <?php echo esc_attr( $button_class ) ?>" data-builder="<?php echo esc_attr( $active_text ) ?>" data-editor="<?php echo esc_attr( $inactive_text ) ?>" href="" id="tie-page-builder-button"><?php esc_html_e( $button_text ); ?></a>
